...
async def create_applicant(
    ...
    applicant = await applicants_service.create_applicant(data, created_by=current_user.id)
...
async def list_applicants(
    ...
    applicants, total = await applicants_service.list_applicants(limit=limit, offset=offset, q=q)
...
async def get_applicant(applicant_id: UUID) -> ApplicantRead:
    applicant = await applicants_service.get_applicant(applicant_id)
...
async def update_applicant(applicant_id: UUID, data: ApplicantPatch) -> ApplicantRead:
    applicant = await applicants_service.get_applicant(applicant_id)
    if applicant is None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Applicant not found")
...
async def delete_applicant(applicant_id: UUID) -> None:
    applicant = await applicants_service.get_applicant(applicant_id)
    if applicant is None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Applicant not found")

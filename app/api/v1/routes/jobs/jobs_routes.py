...
async def _require_company(company_id: UUID) -> Company:
    # Enforce the `company_id` foreign key: reject references to companies that
    # don't exist rather than allowing orphaned jobs. Returns the company so the
    # caller can embed it in the response without a second lookup.
...
async def create_job(data: JobCreate) -> JobRead:
    company = await _require_company(data.company_id)
    job = await jobs_service.create_job(data)
...
async def list_jobs(
    ...
    jobs, total = await jobs_service.list_jobs(
        limit=limit, offset=offset, q=q, company_id=company_id, status=status
    )
    companies = await jobs_service.get_companies_map(jobs)
    return JobList(
        total=total,
        limit=limit,
        offset=offset,
...
async def get_job(job_id: UUID) -> JobRead:
    ...
    company = await companies_service.get_company(job.company_id)
    return _to_read(job, company)


@router.patch("/{job_id}", response_model=JobRead)
async def update_job(job_id: UUID, data: JobPatch) -> JobRead:
    job = await jobs_service.get_job(job_id)
    if job is None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Job not found")
    if data.company_id is not None:
        await _require_company(data.company_id)
